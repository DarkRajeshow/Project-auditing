...
const Navbar = () => {
    ...
    return (
        <nav className="bg-zinc-200 backdrop-blur-md border-b-2 border-b-black/40">
            <div className="sm:text-base text-sm  container mx-auto px-4 py-5 font-semibold">
                <div className="flex justify-between items-center text-sm sm:text-base">
                    <div className="flex space-x-4 ">
                        <Link to="/" className={`text-black/80 hover:text-blue-800 ${location.pathname == '/' && 'border-b-2 border-b-black/80'}`}>
                            Home
                        </Link>
                        <button onClick={initializeProjectAndNavigate} className={`text-black/80 hover:text-blue-800 shadow-none ${location.pathname.includes("/projects") && 'border-b-2 border-b-black/80 '}`}>
                            Add project
                        </button>
                    </div>
                    <div className="flex space-x-4">
                        {isAuthenticated ? (
                            <>
                                <p className="hidden sm:flex text-black/80 items-center justify-center gap-0.5 sm:gap-1 bg-blue-300/40 px-2 sm:px-3 py-0.5 sm:py-1 rounded-md  transition-all cursor-pointer shadow-[1px_1px_0_0] shadow-black hover:shadow-none">
